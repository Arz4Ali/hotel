...
//     const [rooms, setRooms] = useState(roomCommonData)

//     return (
//         <div>
//             <Title align='left ' font='outfit' title="Room Listings" subTitle='View , edit, or manage all listed rooms.Keeps the information up-to date to provide the bbest experience for users.' />
...
//                                         <label className='relative inline-flex items-center cursor-pointer text-gray-900 gap-3'> <input type="checkbox" className='sr-only peer' checked = {item.isAvailable} />
                    ...
                    <tbody className='text-sm'>
                        {
                            rooms.map((item, index) => (
                                <tr key={index}>
                                    <td className='py-3 px-4 text-gray-700 border-t border-gray-300'>{item.roomType}</td>
                                    <td className='py-3 px-4 text-gray-700 border-t border-gray-300 max-sm:hidden'>  {item.amenities ? item.amenities.join(', ') : 'No amenities listed'}</td>
                                    <td className='py-3 px-4 text-gray-700 border-t border-gray-300'>{item.pricePerNight}</td>
                                    <td className='py-3 px-4 border-t border-gray-300 text-sm text-red-500 text-center'>
